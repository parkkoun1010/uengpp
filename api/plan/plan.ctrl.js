...
const list = (req, res, next) => {
    ...
    jwt.verify(token, "secretKey", (err, _id) => {
        ...
        UserModel.findOne({"token": token},{_id:0, email:1, name:1},(err, result)=>{
          if(!err){
            PlanModel.find({ email: result.email},(err,result) => {
                if (err) return res.status(500).end(); //next(err);
                res.render("plan/list",{ result });
            }).limit(limit)
            .sort({starttime: -1})
          }
        })
...
const detail = (req, res) => {
    ...
    PlanModel.findOne({ _id: id }, (err, result) => {
        if (err) return res.status(500).end(); //throw err
        if (!result) return res.status(404).end();
        //res.json(result);
        res.render("plan/detail",{ result });
    })
};
    ...
    jwt.verify(token, "secretKey", (err, _id) => {
      ...
      UserModel.findOne({"token": token},{_id:0, email:1, name:1},(err, result)=>{
        if(!err){
          ...
          if (!email||!subject || !starttime || !finishtime ||!requiredtime) return res.status(400).send("필수항목이 입력되지 않았습니다.");
      
          PlanModel.create({ email, subject, starttime, finishtime, requiredtime }, (err, result) => {
...
const update = (req, res) => {
    ...
    PlanModel.findByIdAndUpdate(
        id,
        { subject, starttime, finishtime, requiredtime },
        { new : true},
        (err, result) => {
...
const remove = (req, res) => {
    ...
    // 삭제 처리
    PlanModel.findByIdAndDelete(id, (err, result) => {
...
const showUpdatePage = (req, res) => {
    ...
    PlanModel.findById(id, (err, result) => {
        if (err) return res.status(500).send("조회 시 오류가 발생했습니다.");
        if (!result) return res.status(404).send("해당하는 정보가 없습니다.");
        res.render("plan/update",{result});
    });
};
